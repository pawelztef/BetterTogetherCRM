  ...
  context 'method responsivnes' do
    ...
    describe 'method #fullname' do
      it 'respond to #fullname' do  
        expect(donator).to respond_to :fullname
      end
      it '#fullname return proper string' do
        expect(donator.fullname).to match /^[A-Z]\w+\s[A-Z]\w+$/
      end
    end

    describe "megthod #remove_white_spaces" do
      it 'repsond to #remove_withe_spaces method' do
        expect(donator).to respond_to :remove_white_spaces
      end
      it 'return corect string' do
        donator.remove_white_spaces
        expect(donator.first_name).not_to match /\s/
        expect(donator.last_name).not_to match /\s/
        expect(donator.email).not_to match /\s/
      end
    end
